...
interface MeetingMinutesData {
    ...
    mentorship: {
        ...
        mentees?: { mentee: { id: string; firstName: string; lastName: string; avatar: string | null } }[];
    } | null;
    minutes: {
        id: string;
        status: string;
...
export function MinutesManager({ meetings }: { meetings: MeetingMinutesData[] }) {
    ...
    const menteeMap = useMemo(() => {
        const map = new Map<string, { mentee: any; meetings: MeetingMinutesData[] }>();
        // Check for general meetings without a mentee mapping (e.g. general admin view)
        const unassignedMentees: MeetingMinutesData[] = [];
    ...
    const [activeTab, setActiveTab] = useState<string | "all">(menteesList[0]?.mentee.id || "all");
    ...
    let displayedMeetings = activeTab === "all" ? meetings : menteesList.find(m => m.mentee.id === activeTab)?.meetings || [];
        ...
        <div className="space-y-4">
            ...
            {menteesList.length > 0 && (
                <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
                    <TabsList className="bg-muted/50 p-1 flex-wrap h-auto mb-4">
            ...
            <MeetingList meetings={displayedMeetings} />
        </div>
    );
}

function MeetingList({ meetings }: { meetings: MeetingMinutesData[] }) {
                            ...
                            >
                                <div className="min-w-0">
                                    <p className="text-[13px] font-semibold text-foreground truncate">
                                        {meeting.sessionNumber && !meeting.title.startsWith("Buổi") ? <span className="text-primary mr-1">Buổi #{meeting.sessionNumber}:</span> : ""}
                                        {meeting.title}
                                    </p>
                                </div>
