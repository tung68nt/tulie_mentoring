...
export function WikiTreeSidebar({ structure }: WikiTreeSidebarProps) {
    ...
    const [expandedCats, setExpandedCats] = useState<Set<string>>(() => {
        ...
        structure.forEach(cat => {
            cat.wikis.forEach(wiki => {
                if (pathname.includes(wiki.slug)) expanded.add(cat.id);
                wiki.pages.forEach(page => {
                    if (pathname.includes(page.slug)) expanded.add(cat.id);
                });
            });
        });
        return expanded;
    ...
    const [expandedWikis, setExpandedWikis] = useState<Set<string>>(() => {
        ...
        structure.forEach(cat => {
            cat.wikis.forEach(wiki => {
                wiki.pages.forEach(page => {
                    if (pathname.includes(page.slug)) expanded.add(wiki.id);
                });
            });
        });
        return expanded;
    ...
    return (
        <div className="w-72 flex-shrink-0 border-r border-border/40 bg-muted/5 flex flex-col transition-all duration-200 h-full overflow-hidden">
            {/* Header */}
            ...
            <div className="px-3 py-3">
                <div className="relative">
                    ...
                    <Input
                        ...
                        className="h-8 pl-8 text-sm bg-muted/30 border-transparent focus:border-primary/20 focus:bg-background shadow-none rounded-md"
                    />
                </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar px-2 pb-4 space-y-1">
                {filteredStructure.map(cat => {
                    ...
                    return (
                        <div key={cat.id}>
                            ...
                            <div className="group flex items-center">
                                {editingCategory === cat.id ? (
                                    ...
                                    <button
                                        ...
                                        className="flex-1 flex items-center gap-2 px-2 py-2 rounded-md text-left hover:bg-muted/50 transition-colors"
                                    >
                                        {catExpanded
                                            ? <ChevronDown className="w-3.5 h-3.5 text-muted-foreground/50 shrink-0" />
                                            : <ChevronRight className="w-3.5 h-3.5 text-muted-foreground/50 shrink-0" />
                                        }
                                        <CatFolderIcon className="w-4 h-4 text-muted-foreground/50 shrink-0" />
                                        <span className="text-sm font-semibold text-foreground/80 truncate flex-1">{cat.name}</span>
                                        <span className="text-xs text-muted-foreground/40 font-medium">{cat.wikis.length}</span>
                                    </button>
                                )}
                                ...
                                {editingCategory !== cat.id && (
                                    <DropdownMenu>
                                        <DropdownMenuTrigger asChild>
                                            <button className="h-6 w-6 rounded flex items-center justify-center text-muted-foreground/20 opacity-0 group-hover:opacity-100 hover:text-foreground hover:bg-muted transition-all shrink-0">
                            ...
                            {catExpanded && (
                                <div className="ml-4 pl-3 border-l border-border/30 space-y-0.5 my-1">
                                    {cat.wikis.map(wiki => {
                                        ...
                                        return (
                                            <div key={wiki.id}>
                                                ...
                                                <div className="group/wiki flex items-center">
                                                    {editingWiki === wiki.id ? (
                                                        ...
                                                        >
                                                            ...
                                                            <span className="text-xs text-muted-foreground/40">{wiki.pages.length}</span>
                                                        </button>
                                                    )}
                                                    ...
                                                    {editingWiki !== wiki.id && (
                                                        <DropdownMenu>
                                                            <DropdownMenuTrigger asChild>
                                                                <button className="h-6 w-6 rounded flex items-center justify-center text-muted-foreground/20 opacity-0 group-hover/wiki:opacity-100 hover:text-foreground hover:bg-muted transition-all shrink-0">
                                                                    <MoreHorizontal className="w-3.5 h-3.5" />
                                                                </button>
                                                            </DropdownMenuTrigger>
                                                            <DropdownMenuContent align="end" className="w-44">
                                                ...
                                                {/* Pages inside wiki */}
                                                {wikiExpanded && (
                                                    <div className="ml-4 pl-2.5 border-l border-border/20 space-y-0.5 my-0.5">
                                                        {wiki.pages.map(page => {
                                                            ...
                                                            return (
                                                                <Link
                                                                    ...
                                                                    className={cn(
                                                                        "flex items-center gap-2 px-2 py-1.5 rounded-md text-sm transition-all group/page",
                                                                        isPageActive
                                                                ...
                                                                >
                                                                    <FileText className={cn(
                                                                        "w-3.5 h-3.5 shrink-0",
                                                                        isPageActive ? "text-primary" : "text-muted-foreground/40 group-hover/page:text-muted-foreground/60"
                                                                    )} />
                                                        ...
                                                        <Link
                                                            ...
                                                            className="flex items-center gap-2 px-2 py-1.5 rounded-md text-xs text-muted-foreground/40 hover:text-primary hover:bg-primary/5 transition-colors"
                                                        >
                                    ...
                                    {addingWikiInCat === cat.id && (
                                        <div className="px-1 py-1">
                                            <Input
                                                ...
                                                className="h-7 text-sm px-2 shadow-none"
                                                disabled={isPending}
                                            />
                                        </div>
                                    )}
                ...
                {filteredStructure.length === 0 && (
                    <div className="px-3 py-8 text-center">
                        <p className="text-xs text-muted-foreground/50">Không tìm thấy.</p>
                    </div>
                )}
            </div>
                ...
                {addingCategory ? (
                    <div className="flex items-center gap-1.5">
                        <Input
                            ...
                            className="h-8 text-sm px-3 shadow-none flex-1"
                            disabled={isPending}
                        />
                    ...
                    <button
                        ...
                        className="flex items-center gap-2 px-2 py-2 rounded-md text-sm font-medium text-muted-foreground/50 hover:text-primary hover:bg-primary/5 transition-colors w-full"
                    >
